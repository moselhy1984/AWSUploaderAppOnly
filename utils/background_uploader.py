...
class BackgroundUploader(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            uploaded_files_db = self.get_uploaded_files(self.order_number)
            uploaded_file_keys = set(uploaded_files_db)
            processed_files = 0
            uploaded_files = 0
            skipped_files = 0
            for file_info in all_files:
                ...
                if s3_key in uploaded_file_keys:
                    try:
                        s3_response = s3.head_object(Bucket=self.aws_session.bucket_name, Key=s3_key)
                        if s3_response['ContentLength'] == file_size:
                            self.log.emit(f"File verified in S3: {os.path.basename(local_path)}")
                            skipped_files += 1
                            processed_files += 1
                            self.progress.emit(processed_files, total_files)
                            continue
                        else:
                            self.log.emit(f"File size mismatch in S3, will re-upload: {os.path.basename(local_path)}")
                    except ClientError:
                        self.log.emit(f"File not found in S3, will upload: {os.path.basename(local_path)}")
                # Upload the file
                try:
